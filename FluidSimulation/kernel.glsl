...
float wpoly6(float r) {
	return (315.0 / 64.0 / pi * h9inv) * pow(h2 - r * r, 3.0);
}

vec3 wpoly6grad(vec3 r) {
	return (-945.0 / 32.0 / pi * h9inv) * pow(h2 - dot(r, r), 2.0) * r;
}

float wpoly6lapl(float r) {
	return (-945.0 / 32.0 / pi * h9inv) *  
		(3.0 * pow(h2 - r * r, 2.0) - 4.0 * r * r * (h2 - r * r));
}

float wspiky(float r) {
	return (15.0 / pi * h6inv) * pow(h - r, 3.0);
}

vec3 wspikygrad(vec3 r) {
	return (-45.0 / pi * h6inv) * pow(h - length(r), 2.0) * r / length(r);
}

float wspikylapl(float r) {
	return (-90.0 / pi * h6inv) * (pow(h - r, 2.0) - (h - r)) / r;
}
